...
Vanagon is a packaging tool to build a single package out of a project, which can itself contain one or more components. A relatively simple [project](https://github.com/puppetlabs/pegasus-vanagon) exists for building [Pegasus](https://github.com/puppetlabs/pegasus). It’s used by Release Engineering for building native code projects, including [Puppet Agent](https://github.com/puppetlabs/puppet-agent) and a number of the libraries and tools mentioned above for [pl-build-tools](https://github.com/puppetlabs/pl-build-tools-vanagon).
